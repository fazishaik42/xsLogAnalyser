...
class GenAIRequestHandler:
    ...
    def xsRequestAI(self,**kwargs):
        ...
        '''
        self.xs_file_filter_obj       =   FileFilter(accept="ERROR",ignore="INFO")
        ...
        '''
        xs_ai_promot             =   self.xs_file_filter_obj.readErrorStatement()
        print(f"No of Error Statements to send to AI : {str(len(xs_ai_promot))}")
        for i in range(0,len(xs_ai_promot)):
            ...
            if response.status_code == 200:
                ...
                self.xs_file_filter_obj.xsWriteAIResponse(data=xs_AI_RESP)
            else:
